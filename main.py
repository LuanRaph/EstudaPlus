...
<Plan_screen>:
	...
	FloatLayout:
		...
		MDFillRoundFlatButton:
			text: 'Selecionar'
			pos_hint: {'center_x': 0.83, 'center_y': 0.76}
			font_size: "11dp"
			md_bg_color: "white"
			text_color: "black"
			line_color: "#cf0a0a"
...
class EstudaPlus(MDApp):
	def build(self):
		user = self.usuario()
		self.materias_json = []
		self.flash_store = JsonStore(f'flashcards_{user}.json')
		self.flashcards = self.load_flashcards()
		return Builder.load_string(kv)

	def usuario(self):
		if sign.exists('current_user'):
			return sign.get('current_user').get('username')
		return None
	...
	#Captura o valor do tempo escolhido de inicio e final
	...
	def text_hours(self):
		try:
			tempo_inicio = self.root.get_screen('Plan').ids.time_selected.text.replace(":", "")
			tempo_final = self.root.get_screen('Plan').ids.time_selected2.text.replace(":", "")
		except Exception:
			return
		#Pega valor do tempo selecionado, transforma em minutos e substrai inicio pelo final 
		if len(tempo_inicio) != 4 or len(tempo_final) != 4:
			return
		
		try:
			hora_inicio_1 = int(tempo_inicio[0:2])
			hora_inicio_2 = int(tempo_inicio[2:4])
			hora_final_1 = int(tempo_final[0:2])
			hora_final_2 = int(tempo_final[2:4])
			minutos_inicio = hora_inicio_1 * 60 + hora_inicio_2
			minutos_final = hora_final_1 * 60 + hora_final_2
			total_min = minutos_final - minutos_inicio
			if total_min < 0:
				return
			sign.put('time', min=total_min)
		except Exception as e:
			return

	def salvarMateria(self, texto):
		user = self.usuario()
		if not user:
			return
		if texto.strip():
			jsonMaterias = JsonStore(f'materias_{user}.json')
			try:
				self.materias_json = jsonMaterias.get('lista')['items']
			except KeyError:
				self.materias_json = []
			self.materias_json.append(texto.strip())
			jsonMaterias.put('lista', items=self.materias_json)
			self.ListaMaterias()


	def carregarMaterias(self):
		user = self.usuario()
		if not user:
			self.materias_json = [] 
			return
	...
	def ListaMaterias(self):
		user = self.usuario()
		getTime = JsonStore('sign.json')
		dados = getTime.get('time')
		time = dados.get('min')
		jsonMaterias = JsonStore(f'materias_{user}.json')
		try:
			self.materias_json = jsonMaterias.get('lista')['items']
			cont = len(self.materias_json)
			organizacao = round(time / cont)
			self.root.get_screen('Plan').ids.list_m.clear_widgets()
		except KeyError:
			self.materias_json = []
		for materia in self.materias_json:
			item = TwoLineRightIconListItem(text=materia, secondary_text=f"Tempo: {cont} horas" if organizacao % 60 == 0 and organizacao > 0 else f"Tempo {organizacao} minutos")
			self.root.get_screen('Plan').ids.list_m.add_widget(item)
		
		
	def fecharCriacao(self):
		screen = self.root.get_screen("Plan_screen")
    
		if hasattr(self, 'card_materias'):
			screen.remove_widget(self.card_materias)
			del self.card_materias 


	def add_matery(self):
		layout = FloatLayout(
			size_hint=(0.85, None), height=dp(350))
		criacao_materias = MDCard(
			size_hint=(0.85, None),
        	height=dp(400),
		...
		textoMateria = MDLabel(
			text="Crie sua materia",
			pos_hint={'center_x': 0.71,'center_y': 1.05},
			font_style="H5"
			)
		campoMateria = MDTextField(
			mode="round",
			hint_text="Digite o nome da materia",
			pos_hint={'center_x': 0.5, 'center_y': 0.8},
			line_color_normal="white",
			line_color_focus="red",
		)
		self.capture_materia = campoMateria
		...
		buttonCriacao = MDFillRoundFlatButton(
			text="Confirmar", 
			size=(dp(120), dp(50)),
			size_hint=(None, None),
			...
			md_bg_color="red",
			on_press=lambda x: [
				self.salvarMateria(campoMateria.text),
				self.ListaMaterias(),
				self.root.get_screen("Plan").remove_widget(criacao_materias),

			]
			)
		iconFechar = MDIconButton(
			icon="window-close",
			pos_hint={'center_y': 1.07, 'center_x': 0.08},
			on_release=lambda x, card=criacao_materias: self.root.get_screen("Plan").remove_widget(card)
		)
		btn_layout.add_widget(buttonCriacao)
		layout.add_widget(iconFechar)
		layout.add_widget(campoMateria)
		layout.add_widget(textoMateria)
		layout.add_widget(btn_layout)
		criacao_materias.add_widget(layout)
		self.root.get_screen("Plan").add_widget(criacao_materias)
	...
	def motivacion_text(self):
		lista = ["Um passo de cada vez.",
				 ...
				 "O sucesso não cai do céu. Exige luta e esforço.",
				 ]
	...
	def load_flashcards(self):
		...
		try:
			with open(f'flashcards_{user}.json', 'r', encoding='utf-8') as f:
				data = json.load(f)
				return data.get('items', [])
		except Exception:
			return []
	...
	def save_flashcards(self):
		...
		try:
			...
			try:
				with open(f'flashcards_{user}.json', 'w', encoding='utf-8') as f:
	...
	def progress(self):
		dados = sign.get('time')['min']
		dados2 = sign.get('time_gasto',)['tempo']
		tarefas = ProgressBar(max=4, value=2,
							size_hint=(None, None),
							size=(dp(250), dp(50)),
							pos_hint={'center_x': 0.5, 'center_y': 0.7})
		self.root.get_screen('progress').add_widget(tarefas)
		tempo = ProgressBar(value=int(dados2), max=int(dados),
							size_hint=(None, None),
							size=(dp(250), dp(50)),
	...
	def getCronometro(self, min):
		sign.put('time_gasto', tempo=min)
